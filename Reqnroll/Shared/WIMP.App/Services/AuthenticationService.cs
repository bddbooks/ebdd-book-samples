...
/// </summary>
public class AuthenticationService(IDataRepository repository, ILogger<AuthenticationService> logger, MessageService messageService)
{
    ...
    public ServiceResult<string> Login(string customerName, string password)
    {
        ...
        {
            return ServiceResult<string>.Failure(messageService.GetMessage("en-US", "invalid-password"));
        }

        string token = Guid.NewGuid().ToString("N");
        repository.InsertSession(new Session(token, customerName));
        logger.LogInformation("{CustomerName} authenticated", customerName);
